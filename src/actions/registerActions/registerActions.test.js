import { registerActions, registerRequest, registerSuccess } from "./registerActions"
  ...
  describe('register user', () => {
    ...
    beforeEach(() => {
      ...
      mockRegister = jest.fn((user) => {
        if (user.username === correctFilledUser.username && user.password === correctFilledUser.password) {
    ...
    it('register user with correct credentials', async () => {
      ...
      expect(dispatch.mock.calls[1][0]).toEqual(registerSuccess(correctFilledUser.username));
    });

    it('register user with weak password', async () => {
      await registerActions.registerUser({ username: 'test', password: 'test' })(dispatch);
      expect(dispatch.mock.calls[0][0]).toEqual({ "type": "REGISTER_REQUEST" });
      expect(dispatch.mock.calls[1][0]).toEqual({ "message": "Register Error", "type": "REGISTER_FAILURE" });
    });
  });
});
